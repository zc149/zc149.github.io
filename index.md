...
- 기간: 2026.05
- 직무: DevOps
- 기술스택: GitHub Actions ARC, Kubernetes, AWS
...
- **S3 기반 Gradle 캐시 적용**
  - 기존 EFS 기반 저장 구조를 레포별 S3 경로 기반 스트리밍 방식으로 전환

- **ARC Runner 기반 캐시 라이프사이클 제어**
  - GitHub Actions Runner Hook을 활용하여 Job 시작 시 캐시 복원, Job 종료 시 캐시 저장 자동화
  - Runner Pod 생명주기와 캐시 동기화 시점을 분리하여 워크플로우 수정 범위 최소화
  - 서비스별 workflow에 반복 로직을 추가하지 않고 Runner 레벨에서 공통 캐시 정책 적용

- **성과**
  - 빌드 시간 약 39% 단축
  - EFS 대비 운영 리소스 및 스토리지 비용 부담 완화
